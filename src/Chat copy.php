...
class Chat implements MessageComponentInterface {
    ...
    public function onOpen(ConnectionInterface $conn) {
        ...
        $conn->send(json_encode(['action'=>'init', 'previous'=>$lobbyCodes, 'error'=>false]));
    ...
    public function onMessage(ConnectionInterface $from, $msg) {
        ...
        if (preg_match('/^\/(create|closed_create)(?:\s+(.+))?/', $msg, $matches)) {
            ...
            $from->send(json_encode(['action'=>'create', 'message'=>"Lobby created with code $code", 'code'=>$code, 'error'=>false]));
        } 
        // Check if the message is a command to join a lobby
        elseif (preg_match('/^\/join\s+([A-Z0-9]{6})(?:\s+(.+))?$/', $msg, $matches)) {
            $code = $matches[1];
            $password = $matches[2] ?? null;
            if (isset($this->lobbies[$code])) {
                if ($this->lobbies[$code]['hasPassword'] && $password !== $this->lobbies[$code]['password']) {
                    $from->send(json_encode(['action'=>'join', 'message'=>"Wrong password", 'error'=>true]));
                    return;
                }
                ...
                $from->send(json_encode(['action'=>'join', 'message'=>"Lobby with code $code not found", 'error'=>true]));
            }
        }
        // Check if the message is a command to close the lobby
        elseif (preg_match('/^\/close\s+([A-Z0-9]{6})$/', $msg, $matches)) {
            $code = $matches[1];
        ...
        elseif (preg_match('/^\/setalias\s+([A-Z0-9]{6})\s+(.+)$/', $msg, $matches)) {
            $code = $matches[1];
            $alias = $matches[2];
            if(isset($this->lobbies[$code])){
                if ($this->lobbies[$code]['clients']->contains($from) || ($this->lobbies[$code]['isClosed'] && $this->lobbies[$code]['waiting_room']->contains($from))) {
                    if($this->isAliasInUse($code, $alias)){
                        $from->send(json_encode(['action'=>'alias', 'message'=>"Alias `$alias` already in use!", 'error'=>true]));
                    }
                    else{
                        ...
                        $from->send(json_encode(['action'=>'alias', 'message'=>"Your alias has been set to $alias", 'error'=>false]));
                        $this->lobbies[$code]['lastActiveTime'] = time();
                    }
                }
            }
        ...
        elseif(preg_match('/^\/risehand\s+([A-Z0-9]{6})(?:\s+(.+))?$/', $msg, $matches))
        {
            $code = $matches[1];
            $message = $matches[2] ?? '';
            ...
            if ($this->isValidMember($code, $from)) {
                ...
                if(!isset($alias))
                {
                    $from->send(json_encode(['action'=>'risehand', 'message'=>"Please set a alias.", 'error'=>true]));
                }
                else{
                    $lobby['owner']->send(json_encode(['action'=>'risehand', 'user'=>$alias, 'message'=>$message, 'error'=>false]));
                    $from->send(json_encode(['action'=>'risehand', 'message'=>"Success: Your hand is raised!", 'error'=>false]));
                    $lobby['lastActiveTime'] = time();
                }
            }
        }
        // Check if the message is a command to check queue of a lobby
        elseif (preg_match('/^\/queue\s+([A-Z0-9]{6})$/', $msg, $matches)) {
            $code = $matches[1];
            if (isset($this->lobbies[$code]) && $this->lobbies[$code]['isClosed'] && $this->lobbies[$code]['owner'] === $from) {
                ...
                foreach ($this->lobbies[$code]['waiting_room'] as $conn) 
                {
                    ...
                    $waitingList[] = ['alias' => $alias, 'conn' => $conn->resourceId];
                }

                // Send the waiting list to the lobby owner
                $this->lobbies[$code]['owner']->send(json_encode(['action'=>'waiting_list', 'list'=>$waitingList, 'error'=>false]));
            }
        }
        // Check if the message is a command to check queue of a lobby
        elseif (preg_match('/^\/members\s+([A-Z0-9]{6})$/', $msg, $matches)) {
            $code = $matches[1];
            if (isset($this->lobbies[$code]) && $this->lobbies[$code]['owner'] === $from) {
                $memberList = [];
                ...
                // Send the waiting list to the lobby owner
                $this->lobbies[$code]['owner']->send(json_encode(['action'=>'members_list', 'list'=>$memberList, 'error'=>false]));
            }
        }
        // Check if the message is a command to close the lobby
        elseif (preg_match('/^\/accept\s+([A-Z0-9]{6})(?:\s+(\d+))$/', $msg, $matches)) {
            $code = $matches[1];
            ...
            if (isset($this->lobbies[$code]) && $this->lobbies[$code]['isClosed'] && $this->lobbies[$code]['owner'] === $from) {
                foreach ($this->lobbies[$code]['waiting_room'] as $index => $conn) 
                {
                    ...
                    if ($conn->resourceId == $connectionID) {
                        ...
                        $this->lobbies[$code]['owner']->send(json_encode(['action'=>'accept', 'message' => "$connectionID has been added!", 'error'=>false]));
                        break;
                    }
