...
class CommonElements
{
    ...
    public function __construct()
    {
        ...
        $this->owner =  new ConnectionInterface;
        $this->lastActiveTime = time();
        $this->aliases = [];
        $this->hasPassword = false;
        $this->password = "";
        $this->isClosed = false;
        $this->waiting_room = new \SplObjectStorage;
    }
}
    ...
    public function onMessage(ConnectionInterface $from, $msg) {
        ...
        elseif (preg_match('/^\/queue\s+([A-Z0-9]{6})$/', $msg, $matches)) {
            ...
            if (isset($this->lobbies[$code]) && $this->lobbies[$code]['isClosed'] && $this->lobbies[$code]['owner'] === $from) {
                ...
                $this->lobbies[$code]['owner']->send(json_encode(['action'=>'queue', 'list'=>$queueList, 'error'=>false]));
            }
        }
        // Check if the message is a command to check queue of a lobby
